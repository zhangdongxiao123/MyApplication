...
public class LoginActivity extends AppCompatActivity {
    ...
    public void setActionBar(){
        ...
        actionBar.setIcon(R.mipmap.littlelogo);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if(item.getItemId()==R.id.menu_exit){
            ...
            builder.setIcon(R.mipmap.littlelogo);
    ...
    public void init(){
        ...
        SharedPreferences loginStaus=getSharedPreferences("loginStaus",MODE_PRIVATE);//只能自己APP使用
        ...
        login.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                ...
                if(str_uname.equals("admin")&&str_upwd.equals("123456")){
                    ...
                    if(rememberpwd.isChecked()){   //数据存储
                        editor.putString("uname",str_uname);  //输入名字
                        editor.putString("upwd",str_upwd);     //输入密码
                        editor.putInt("status",1);  //1表示打勾，将输入的名字和密码存储起来
                    }else
                    {
                        editor.clear();  //若没有打勾则清理数据
                    }
                    editor.commit();
